...
LocalNucleoInterface::~LocalNucleoInterface() {
    close();
}
...
template <typename T>
void append_to_vector_le(std::vector<uint8_t>& vec, T value) {
...
template <typename T>
T read_from_vector_le(const std::vector<uint8_t>& vec, size_t offset) {
    T value;
#if __BYTE_ORDER__ == __ORDER_LITTLE_ENDIAN__
